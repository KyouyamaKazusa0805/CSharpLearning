...
只要出现了一个 `var _` 或者别的什么类型，后面 `and` 模式可以增加附带的 `_` 而且不带有别的任何的情况，直接写下划线。当然，这么写没有意义，因为是冗余的模式匹配（它什么都没有判定）。
...
#### `null` 对于 `ReadOnlySpan<char>` 的奇怪表现
...
* [`is` 运算符重载](https://github.com/dotnet/csharplang/issues/1047)：允许自定义 `operator is` 的语法来完成自定义 `is` 匹配规则；
* [自定义模式匹配](https://github.com/dotnet/csharplang/issues/4131)：允许方法等成员放在 `is` 后进行匹配的规则；
* [针对 `IEnumerable<T>` 类型对象的列表模式匹配](https://github.com/dotnet/csharplang/blob/main/proposals/list-patterns-enumerables.md)：对于 `IEnumerable<T>` 类型由于目前没有索引器支持所以它不支持列表模式，该提案将它翻译成 .NET API 方法调用来支持列表模式匹配；
* [`or` 和 `not` 模式下的变量定义增强](https://github.com/dotnet/csharplang/blob/main/proposals/pattern-variables.md)：允许在包含 `or` 模式的两侧定义变量；
* [索引器模式](https://github.com/dotnet/csharplang/discussions/4889)：允许使用类似索引器的调用方式来判断属性规则的新匹配模式；
* UTF-8 字面量：允许给字符串末尾加一个 `u8` 或 `U8` 后缀，使得它是 UTF-8 编码的字符串（例如 `"Hello"u8` 这样的语法）。虽说这个跟模式匹配没啥大关系，但是以后说不定会创建单独的数据类型，于是可能会有针对于这种类型的模式匹配语法。

稍微提及一下这些特性吧。

### `is` 运算符重载（Overloadable operator `is`）
...
### 自定义模式匹配（User-defined Positional Pattern）

允许用户定义一个方法，使得该方法满足一定的模式匹配要求，这样的方法就可以放在 `is` 之后了。假设我这样写代码：

```csharp
public bool Odd(this int i)
{
    return (i & 1) != 0;
}
```
...
```csharp
public bool HasFlag(this int i, int flag)
{
    return (i & flag) == flag;
}
```
...
这些都不行。因为目前的 C# 类型体系不支持我们使用类似 `int a or double b` 这样的“两者选择其一”的语法规则。该语法在 C# 里是有提案的，叫做“可区分联合体”（Discriminated Union），在别的编程语言里有，比如 F#。但是，C# 并不支持该特性。

如果有了这个玩意儿的话，想必上面的这种定义模式说不定就有可能存在了。

### 索引器模式（Indexer Pattern）
...
### UTF-8 字面量（UTF-8 String Literals）

该特性跟模式匹配关系不大，因此就不在这里说了。不过你应该猜得到，它以后如果有了的话，肯定会有对应的数据类型，比如叫做 `Utf8String` 和 `Utf8Char`。

现已知的是，C# 正在考虑这个特性，但是目前因为没有此类型可以对应，所以目前这种字面量是直接赋值给 `ReadOnlySpan<byte>` 或 `byte[]?` 类型的。毕竟，UTF-8 格式的话，只有 256 种情况，这刚好兼容的是 `byte` 类型的范围。你想想，C 语言里 `byte` 没有这种类型，不也用的是 `unsigned char` 来代替的吗？它们是可转换的。
